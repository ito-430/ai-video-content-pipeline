...
特殊能力・ステージ・ルール等の新しいアイデアが継続的に生まれる仕組みとして設計した。
ただし実際に安全に自動化できるのは、既存の物理演算コード(simulate())が
...
# 尺の要件「30〜60秒(ごくたまに数秒)」の"ごくたまに"は少数派であるべきなので、
...
- 特殊能力(8種、色ごとに固定): dash/poison_wire/shockwave/vortex/growth_surge/teleport/
  ...
  効果は今後も導入しない方針です**(終盤の見栄えを悪くする演出だと判断したため)。
...
# プールの各コンボは元々uses/total_score(平均reward=total_score/usesが実績)を持っていたが、
# これを実際に使って選ぶロジックが無く「生成して眠らせるだけ」だった。イプシロン-グリーディ
# 方式で、
