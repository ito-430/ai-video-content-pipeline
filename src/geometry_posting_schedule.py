"""SimuSphere Arena(4ch目)の投稿タイミング判定。

[[project_geometry_battle_channel]]の投稿枠は「01:00/09:00/21:00 JSTの固定3枠」で確定済みであり、
...
- GitHub Actionsのscheduleトリガーは低頻度リポジトリだと数時間規模で遅延・間引きされることが
  実測で分かっている([[project_infra]]の2026-07-14の記録、[[project_geometry_battle_channel]]の
  2026-09-09の記録で同じ現象を確認)。そのため主たる起動経路は外部cronサービスからの
