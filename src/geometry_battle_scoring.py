"""
幾何学アニメーション物理演算バトルチャンネル(4ch目) 基準判定ロジック プロトタイプ

軽量シミュレーション結果(geometry_battle_gen.SimResult)を受け取り、8つの評価軸
(インプレッション分類/サプライズ指数/尺/気持ちよさ/密度・視認性/停滞検知/勝率バランス/
ブランド一貫性)に対する加重スコアへ変換し、80%以上の一致度で合格/不合格を判定する。
...
重み付けは初期値であり、実データを見ながら継続的に調整する前提。
"""
...
# 基準判定の合否しきい値。8項目の加重スコアがこれを下回ったら不合格とする
DEFAULT_THRESHOLD = 0.8

# 尺フィルター: 決着までの秒数を15〜25秒に制限する無条件の足切り。DURATION_CATEGORIESは
# 他項目とのバランスを見る加点方式のスコアで、極端に短い/長い尺でも他項目が良ければ
# 合格しうる設計だが、「5秒で即決着」のような動画は他項目のスコアがどれだけ高くても
# 視聴体験を損なうため、加点方式とは別に無条件の足切りとして分離した。上限を短めの
# 25秒に設定しているのは、ループ再生されるShorts特性上、尺が短いほど視聴維持率を
# 稼ぎやすいため。
PRODUCTION_DECISION_SECONDS_RANGE = (15.0, 25.0)

# 序盤(離脱の大部分が発生する0〜3秒)の情報密度を上げるため、開始からこの秒数以内に
# 最初の衝突(プレイヤー間または壁/障害物との衝突、いずれもresult.collisionsに記録される)
# が発生しない候補は「序盤停滞」として無条件不合格にする(尺フィルター・no_interaction
# フィルターと同種の無条件足切り)。
FIRST_IMPACT_MAX_SECONDS = 1.5

# drama(0.20)は決着までの駆け引き・逆転の起きやすさを評価する軸として新設。他項目を
# 比例縮小して枠を確保した(density=0.10→0.075、他の非プレースホルダー項目も若干縮小)。
# 合計は1.0を維持する。
SCORE_WEIGHTS = {
    "impression": 0.125,
    "surprise": 0.125,
