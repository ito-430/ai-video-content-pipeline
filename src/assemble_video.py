...
CHARACTER_FOLDER = {"touma": "Touma", "yuzu": "Yuzu", "sora": "Sora", "kai": "Kai", "dady": "Dady"}
FILE_PREFIX = {"touma": "touma", "yuzu": "yuzu", "sora": "sora", "kai": "kai", "dady": "dady"}
...
CHARACTER_LABEL = {"touma": "トウマ", "yuzu": "ユズ", "sora": "ソラ", "kai": "カイ", "dady": "覆面・ダディ"}
...
CUTIN_SIDE = {"sora": "left", "kai": "right"}
...
def _fit_bubble_text(box_w: int, box_h: int, text: str):
    ...
    セリフが吹き出しの輪郭からはみ出す事故を防ぐため、外接矩形いっぱいを使うのをやめ、
    楕円に内接する矩形(各辺を約1/√2倍)を実際の文字使用領域にした。これにより
    丸みのある吹き出しの輪郭にかからなくなる。
    """
...
def _filter_plausible_bubbles(
    ...
    必要数ちょうどの吹き出しが検出されていても、そのうち1つの座標が実際にはキャラの
    首元の陰や背景を指しており、そこにセリフ文字とdark_only消去が乗って「顔(周辺)に
    モザイク+セリフ」になり、代わりに本来セリフが入るべき正しい吹き出しが「未使用」として
    消去される事故が実際の生成画像(panel11.png、touma2個目の検出box)で確認された。
    個数さえ合っていれば通っていた従来のshortfallチェック(発話者ごとの検出数のみを見る)
    では検知できないため、座標の中身(白い塗りつぶしピクセルの割合)を見て機械的に
    ふるい落とす。吹き出しは常に白地に黒縁で描かせているため、本物の吹き出しなら
    大部分が白いピクセルになる。
    """
...
def _assign_bubble_boxes(
    ...
    # 吹き出しが足りない(検出漏れ)時のフォールバック位置を、読み順だけを見て均等に
    # 帯を割り振る方式にすると、既に検出できている別の吹き出しと重なってしまい
    # 「文字の挿入箇所がなくなる」ことがあった。実際に検出できている全吹き出しの
    # 位置(occupied_bands)を避けるように、フォールバック位置を動的に探す。
...
def _generate_panel_image(
    ...
    背景・キャラクター・吹き出しを分離して生成する方式から、1枚の絵として一括生成する
    方式に変更した(頻度を週1〜2に落とす代わりに、1コマあたりの生成回数も1回
    (+吹き出し位置検出の軽いテキスト呼び出し1回)まで削減する)。
    ...
    def _tail_point(bubble: dict):
        # 吹き出しの外接矩形の中心だけで発話者を推定すると、複数人が近接する構図で
        # 誤って隣のキャラクターに割り当たることがあった。しっぽの先端(発話者を明確に
        # 指す一番細い点)の座標を明示的に検出させ、それを発話者判定の基準にする方が、
        # 外接矩形中心より人間の読み方に近く頑健(自己申告方式より機械的)。
    ...
    for attempt in range(max_attempts):
        ...
        # AI自身に「どの人物を指しているか」を自己申告させる方式は取り違えが残ったため、
        # 各キャラクターの位置も同時に検出させ、各吹き出しの中心に幾何学的に最も近い
        # キャラクターを発話者とみなす方式に変更した(自己申告より機械的で頑健)。
        ...
        # 個数は合っていても、座標が実際には吹き出しでない(キャラの顔・服・背景等)ことが
        # あるため、白い塗りつぶし面積で機械的にふるい落としてから不足数を判定する
        # (そうしないと誤検出1個が「検出成功」扱いになり、後段でそこにセリフと
        # dark_only消去が乗ってしまう)。
        by_speaker = _filter_plausible_bubbles(by_speaker, panel_img, w, h)

        # 合計数だけの判定だと、特定のキャラクターに吹き出しが偏って(例: 発話者Aに2個・
        # Bに0個)割り当てられても「合計数は足りている」と誤判定してしまい、結果的に
        # セリフと吹き出しの対応が崩れていた。発話者ごとに必要数を満たしているかを
        # 個別にチェックする方式に変更した。
    ...
    #
    # 固定比率(輪郭の1/2.0等)で消すと、セリフが短い時は消しすぎて輪郭に接近し、
    # セリフが長い時は逆に消去範囲より実際の文字がはみ出て「消した範囲」と「文字を
    # 描く範囲」がずれ、境目に不自然な描画が出ていた。`_render_bubble_lines`で実際に
    # 描画される文字のインク外接矩形(行間を含む行の高さではなく実際に色が乗るピクセル
    # だけ)をそのまま使うことで、常に過不足なく消せるようにした。
    #
    # それでもなお、検出された吹き出しの外接矩形がしっぽの分だけ縦に長く実際の
    # 楕円中心とズレることがあり、矩形消去だとしっぽ周辺の背景(空や旗の色)まで
    # 巻き込んでしまっていた。そのため実際の消去(_erase_regions)側でdark_only=True
    # を指定し、この矩形内でも文字インク相当の暗い色のピクセルだけを対象にする
    # (白い吹き出し内部や矩形がはみ出した背景は触らない)二重の安全策にした。
...
def _scroll_transition_clip(
    ...
    静止画のスクロール演出には(1)白い余白(gap)、(2)加速→減速のease-in-out、
    (3)動きの速い区間へのモーションブラーを加えている。余白の高さはコマの高さの
    約半分にした(gap未指定時はh//2を既定値にする)。
...
# BGMの音量変化を加えた、よりシネマティックな編集を行う。コマ内でのパン/ズーム
# (擬似カメラワーク)は演出として試したが、動きが激しすぎる・画像が見切れるという
# 問題があったため廃止し、各コマは出力フレームに収まる静止画として表示する。
# 詳細な設計判断は別途ドキュメント化している。
# ==========================================================================
...
def _pick_caption_y(out_h: int, cap_h: int, occupied_ranges: list[tuple[int, int]]) -> int:
    ...
    単語解説キャプションが2行になりバーの高さが増えると、直前のセリフの吹き出し
    (セリフを言い終えてもコマが終わるまで表示され続ける仕様)と重なって見づらくなる
    ことがある。まず既定位置(画面下寄り)を試し、重なる場合はその中で最も上にある
    吹き出しの上端のすぐ上までバーを押し上げる処理を、空きが見つかるか上限(画面上部)に
...
def draw_caption_bar(w: int, h: int, text: str) -> Image.Image:
    ...
    「文字が小さくて読めない」「長い説明で見切れる」問題に対応するため、(1)フォント
    サイズを引き上げ、(2)幅に収まらなければ折り返し、(3)バーの高さを行数に応じて
    可変にした(旧実装は固定サイズ1行のみで、はみ出した分は単純に画面外に切れていた)。

    フォントサイズをh(フレーム高さ)基準にすると、ショート(9:16、h=1920)では
    読みやすくても、長尺(16:9、h=1080)では同じ比率でも絶対サイズが小さくなりすぎる
    (1920→1080で高さが約半分)。縦横どちらのフレームでも長辺は共通(このプロジェクトでは
    常に1920)なので、max(w, h)基準に変更し、フォーマットによらず同じ絶対サイズに
    なるようにした。
    """
...
def assemble_kobun_scene_video(
    ...
    *,
    ...
    - visual_cache: 現代語版/古文版のような視覚的に同じ場面(scene_description+
      ...
      「現代文編と古文編のShorts2本で同じコマの画像を使う」という要件に対し、従来は
      各`assemble_kobun_scene_video`呼び出し内のローカル変数としてvisual_cacheを
      持っていたため、ショート2本を別々に呼び出す`assemble_kobun_scene_video_all_variants`
      では、1本目(現代文編)で生成した画像を2本目(古文編)が実際には共有できていな
      かった(ディスク上の再利用判定はscene idそのものが一致するファイル名でしか
      探さないため、現代文編のscene2と古文編のscene8のように視覚的にはペアでも
      scene idが違う場合は見つけられず、結果的に古文編側で見た目の異なる画像が
      新規生成されていた)。過去に初回話数で「完全一致」となっていたのは、たまたま
      同じ9:16素材ディレクトリに分割機能導入前の一括生成で全12コマが既に揃っていた
        ...
        for scene in scenes:
            ...
            for rl in resolved_lines:
                if rl.get("type", "dialogue") == "caption":
                    ...
                    # 画面最下部だと見づらいため、もう少し上に配置する。バーが2行になり
                    # 高さが増えると、まだ表示され続けている吹き出しと重なることがあるため、
                    # 衝突回避付きの`_pick_caption_y`で位置を決める。
                ...
                # セリフを言い終えるとすぐ吹き出しの文字が消えると視認しにくいため、
                # 次のコマに切り替わるまで(このコマが終わるまで)表示し続けるようにする。
            ...
            # 吹き出しの上に乗って邪魔になるため廃止した(draw_se_textは削除済み)。
...
def assemble_kobun_scene_video_all_variants(script_path: Path) -> dict[str, Path]:
    ...
    `reuse_existing_images=True`だけでは、ディスク上にそのscene idそのままのファイル
    (例: 古文編のscene8用panel8.png)が無い限り再利用判定が効かない。現代文編のscene2と
    古文編のscene8のように「視覚的には同じ場面だがscene idが違う」ペアは、まっさらな
    新規話数では2本目の生成時点でまだpanel8.pngが存在しないため見つけられず、結果的に
    現代文編と見た目の異なる画像が新規生成されてしまっていた(初回話数で過去に「完全一致」
    となっていたのは、分割機能導入前の一括生成で9:16素材がたまたま全12コマ揃っていた
    ことによる偶然の一致だった)。ショート2本の呼び出しに同じ`visual_cache`辞書を
    明示的に共有させることで、1本目(現代文編)がscene2で生成した画像を、2本目(古文編)の
    scene8が「visual_key(scene_description+characters_in_panel)が一致する」と判定して
    正しく使い回せるようにした。
    """
    data = json.loads(script_path.read_text(encoding="utf-8"))
    scenes = data["scenes"]
